...
describe("singleH1", () => {
  it("passes with exactly one H1", () => {
    const result = singleH1(createContext("<div><h1>Title</h1></div>"));
  ...
  it("fails with two H1s", () => {
    const result = singleH1(
      createContext("<div><h1>Title</h1><h1>Another Title</h1></div>"),
    );
    expect(result).toMatchInlineSnapshot(`
      {
        "score": 3,
  ...
  it("fails without an H1", () => {
    const result = singleH1(createContext("<div></div>"));
    expect(result).toMatchInlineSnapshot(`
      {
        "score": 3,
        "translation": "none",
  ...
  it("counts a content element that is itself an H1", () => {
    const result = singleH1(
      createContext("<h1>Title</h1><p>Text</p>", "body > *"),
    );
...
describe("altAttribute", () => {
  it("passes without images", () => {
    const result = altAttribute(createContext("<div></div>"));
    expect(result).toMatchInlineSnapshot(`
      {
        "score": 9,
        "translation": "na",
  ...
  it("passes with an alt attribute on every image", () => {
    const result = altAttribute(
      createContext('<div><img src="image.jpg" alt="Image description"></div>'),
    );
    expect(result).toMatchInlineSnapshot(`
      {
        "score": 9,
  ...
  it("passes with an empty alt attribute", () => {
    const result = altAttribute(
      createContext('<div><img src="decoration.svg" alt=""></div>'),
    );
...
describe("headingStructureOrder", () => {
  it("passes with an H2 following an H1", () => {
    const result = headingStructureOrder(
      createContext("<div><h1>Title</h1><h2>Subtitle</h2></div>"),
    );
    expect(result).toMatchInlineSnapshot(`
      {
        "score": 9,
  ...
  it("fails with an H3 directly following an H1", () => {
    const result = headingStructureOrder(
      createContext("<div><h1>Title</h1><h3>Subsection without H2</h3></div>"),
    );
    expect(result).toMatchInlineSnapshot(`
      {
        "details": {
  ...
  it("fails with an H2 as the first heading", () => {
    const result = headingStructureOrder(
      createContext("<div><h2>Section</h2></div>"),
    );
  ...
  it("escapes the heading text in the details", () => {
    const result = headingStructureOrder(
      createContext("<div><h1>Title</h1><h3>Tags &amp; &lt;b&gt;</h3></div>"),
    );
...
function createContext(content: string, contentSelector = "div") {
  const htmlDocument = new DOMParser().parseFromString(content, "text/html");
  return { htmlDocument, contentSelector };
}
